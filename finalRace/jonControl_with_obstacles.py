...
class robotRace(object):
    ...
    def obstacle_course(self, obstacle_block, largest_idx, line_markers):
        print('Folowing: ', largest_idx)
        new_idx = largest_idx
        self.bot.setServoPosition(0)
        ...
        if obstacle_block >=0:
            ...
            self.obst_dist_hist.append(distance)
            self.obst_dist_hist.pop(0)
            avg_distance = self.avg(self.obst_dist_hist, 10)
            self.width_hist.append(self.cam.newBlocks[obstacle_block].m_width)
            self.width_hist.pop(0)
            avg_width = self.avg(self.width_hist, 10)
            print('Distance:', avg_distance)
            print('width: ',  self.cam.newBlocks[obstacle_block].m_width)
            # if the robot is close enough to the obstacle do a turn
            if avg_width > 60 and (abs(self.cam.newBlocks[obstacle_block].m_x - self.cam.newBlocks[largest_idx].m_x)<30):
                if largest_idx == 1:
                    #print('Turning left')
                    self.bot.setMotorSpeeds(0.5, 0, 0.3)
                    self.bot.setMotorSpeeds(0.5, 0.5, 0.2)
                    new_idx = 2
                    
                else:
                    #print('Turning right')
                    self.bot.setMotorSpeeds(0, 0.5, 0.3)
                    self.bot.setMotorSpeeds(0.5, 0.5, 0.2)
                    new_idx = 1
                self.width_hist = [0 for i in range(30)]
        else:
